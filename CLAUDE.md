...
| `flags.go`                      | Declared `<arg>`/`<flag>` on both sides of a run: `validateArgsAndFlags` at load, `registerFlag`/`registerConflicts` on the cobra command, `gatherArgs`/`gatherFlags`/`passthroughParse` back out into `.arg`/`.flag`. |
...
| `mock.go`                       | The `<mock>` grammar: `Mock`/`MockInput`/`MockOutput`/`MockRecord`, `buildMock` and its children, `validateMock`. An `<input match=>` compiles to a `regexp` at load. |
| `mockrun.go`                    | The `<mock>` runtime: `resolveMockInputs` (matches each `<input>` against `.rest`, publishes `.mock`), `planMockOutputs` (renders every path before one byte lands, so a `<record>` can name them), `appendMockRecords`, `writeMockFiles`, and `runMock`. `.mock.argv` is the WHOLE command line with the leaf's name first, because a `<record>` is replayed. |
| `mockinstall.go`                | `--install-mocks <dir>`: `collectMockWrappers` walks the tree for every `<mock>` leaf, and `installMockWrappers` writes one executable `#!/bin/sh` script per leaf. A duplicate leaf name is an error, because one script cannot answer for both. |
...
| `watch.go`                      | `--watch` and `watch=`: `parseWatchInterval`, `validateWatch` (load time), `watchInterval` (flag over config, `off`), `runWatch` (terminal and signal setup), `watchLoop` (one frame per run), and `watchPainter` (in-place repaint, height clipping, append-only without a terminal). |
...
| `guards.go`                     | Input guards: the built-in path-segment rule (`segmentPattern`, `safeSegments`), `validatePreconditions`, `inheritedPreconditions` (ancestors first), and `resolveArgPatterns` (a `pattern=` that names a `<var>`, resolved at load time). |
| `mcpstdio.go`                    | MCP over stdio: `stdioTransport` wires stdin and stdout to one ledger of open request ids, so stdin's EOF waits for the answers in flight. See rule 31. |
...
| `cors.go` / `debug.go` / `docs.go` | CORS middleware for MCP HTTP/SSE; verbose/debug logging; the `docs` subcommand (embeds `README.md` and `api.example.xml`, and prints the grammar from `spec.Schema`). |
...
15. **A watch frame is a whole leaf run.** `--watch` loops `runLeafOnce` (`build.go`) and captures each run's stdout and stderr into one buffer, which `watchPainter` draws over the last frame. Nothing is cached between frames. `ttyOverride` (`format.go`) makes `stdoutTTY`/`stdoutSize` report the real terminal while the frame sits in that buffer, so the formatter still picks the terminal representation. `watchable` refuses a `<download>` leaf, and refuses a `confirm` leaf without `--yes`. `watch=` on a command is the default interval, threaded by `buildCommand` only: the MCP server ignores it, because a tool call is one run. `watchInterval` puts `--watch` over it, and `off` or `0` runs once. `validateWatch` rejects a bad value or a `<download>` leaf at load. An inherited interval that reaches a `<download>` leaf is ignored with a warning, since the group's setting was never about that leaf.
...
25. **A step with `until=` polls** (`runStepAction`, `steps.go`). It repeats its own run until the predicate holds. The predicate reads the last body promoted onto the run context (`.status`, plus `.body`). `interval=` never grows, `attempts=` caps the loop, and exhaustion fails the run with the last body. A non-zero exit ends the poll at once. `over=` and `until=` compose, so one invocation polls N jobs.
...
29. **`<preconditions>` accumulate instead of overriding.** `buildCommand` and `collectMCPLeaves` each thread the inherited list. The ancestors' guards run ahead of the node's own. A `<config>`-level guard therefore gates every run in the tree.
30. **A `pattern=` is validation, not CLI help.** `matchToolArgs` (`runnable.go`) applies it to a tool call. `buildToolInputSchema` publishes it. It renders as a template at load time, against the node's vars and `.env`. A pattern must be known before any value arrives.
31. **Stdin's EOF is not a reason to drop an answer.** The jsonrpc2 layer refuses every write once its reader reports EOF. A client that closed stdin right after its last request therefore got no responses and exit 1. `stdioTransport` (`mcpstdio.go`) counts the request ids it reads and the response ids it writes. It holds the EOF back until nothing is outstanding.
...
5. Add it to `api-cli.xsd` in api-cli-spec, with a document that uses it and the resolved form that document produces. Bump the module here once that lands. Document it in `README.md`. Exercise it in `api.example.xml` when an integration test needs it.
