...
14. **Downloads use their own HTTP client.** `downloadClient` carries a connect deadline and a response-header deadline, and no overall timeout. `httpClient`'s 60s cap is an API deadline, and on a download it becomes a ceiling on file size.
