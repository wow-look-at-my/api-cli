...
| `mockrun.go`                    | The `<mock>` runtime: `resolveMockInputs` (walks `.rest`, publishes `.mock`), `planMockOutputs` (renders every path before one byte lands, so a `<record>` can name them), `appendMockRecords`, `writeMockFiles`, and `runMock`. |
