...
func resolveMockInputs(m *Mock, prog string, raw []string, data map[string]any) error {
	...
	// .mock.argv is the WHOLE command line, with the program name at the front.
	// A record is a compile_commands.json entry, and a consumer replays those
	...
	for i := range m.Inputs {
		...
		case in.Default != "":
			...
			// can fall back to a value resolved before it.
...
func planMockOutputs(m *Mock, data map[string]any) ([]mockFile, error) {
	...
	for i := range m.Outputs {
		...
		for _, ctx := range records {
			...
			// Records that render the same path mean an over= whose path
...
// because a real compiler writing into an existing build tree finds the
...
func runMock(m *Mock, prog string, raw []string, data map[string]any, out, errw io.Writer) (int, error) {
	...
	// The records land before the run: a wrapper around a real tool must log
	// the call even when the tool that follows fails, because a failed compile
	// is exactly the compile somebody wants the command line of.
...
// mockWhen renders a predicate. An empty predicate holds, as it does
