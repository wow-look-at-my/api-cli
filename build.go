...
func runLeafOnce(c *cobra.Command, node Command, args []string, vars map[string]any, cmdTmpl *Cmd, request *Request, cwdTmpl, stdinTmpl, confirmTmpl string, formatRef *FormatRef, formats map[string]*Format) error {
	...
	// <run> is the a single exception: that shape is a thin wrapper, which
	// records the call and then runs the real tool. An INHERITED run stays
	// where it is, exactly as it does for a download, so a mock leaf under a
	// parent that declares a run does not fire that run on the way past.
	if node.Mock != nil {
